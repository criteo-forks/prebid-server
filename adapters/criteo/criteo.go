...
func (a *CriteoAdapter) MakeRequests(request *openrtb.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {

	// Early filter unwanted inventory type
	if shouldRejectRequest(request) {
		return []*adapters.RequestData{}, nil
	}
...
func shouldRejectRequest(request *openrtb.BidRequest) bool {

	if request != nil {
		// Bid only on in-app inventory
		if request.App != nil && request.Device != nil && request.Device.IFA != "" {
			return false
		}
	}

	// Should reject if didn't matched any inventory wanted.
	return true
}
