...
func newCriteoRequest(request *openrtb2.BidRequest) (criteoRequest, []error) {
	var errs []error
...
func newCriteoRequestSlots(impressions []openrtb2.Imp) ([]criteoRequestSlot, []error) {
	var errs []error
	...
	var criteoSlots = make([]criteoRequestSlot, int64(len(impressions)))
	...
	for i := 0; i < len(impressions); i++ {
		...
		generatedSlotID, err := uuid.NewV4()
		if err != nil {
			errs = append(errs, err)
			continue
		}
		criteoSlots[i].SlotID = generatedSlotID.String()
		...
		if impressions[i].Banner != nil {
			if impressions[i].Banner.Format != nil {
				...
				for idx, format := range impressions[i].Banner.Format {
					// TODO: handle properly int64 conversion
					criteoSlots[i].Sizes[idx] = newCriteoRequestSize(int64(format.W), int64(format.H))
				}
				...
				criteoSlots[i].Sizes[0] = newCriteoRequestSize(int64(*impressions[i].Banner.W), int64(*impressions[i].Banner.H))
			}
		}
